...
const HomeIcon = () => (
    <svg className="w-6 h-6 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
);

// ── 🚀 FORMULÁRIO ESPECIALIZADO DE LAR TEMPORÁRIO (Com ViaCEP) ──────────────
...
export default function Index({ auth, temporaryHomes = [] }) {
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Lares Temporários</h2>}>
            ...
            {/* Note o "temporaryHomes.data || temporaryHomes" para suportar a paginação que fizemos no controller */}
            <DataBrowser 
                title="Rede de Apoio"
            ...
            <div className="grid grid-cols-1 gap-3">
                {residentsModal.animals.map(animal => (
                    <div key={animal.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        <Link 
                            href={`/animals/${animal.id}`} // Ou link para o dossiê específico se tiver rota
