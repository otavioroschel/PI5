...
// 🛠️ MANTENDO A CORREÇÃO DE ARQUITETURA REACT: Componente no escopo global para manter o foco
...
// ── 🚀 FORMULÁRIO ESPECIALIZADO DE LAR TEMPORÁRIO (Chamada ViaCEP Direta - Não Segura) ──
const TemporaryHomeForm = ({ initialData, onSuccess, onCancel }) => {
    ...
    const fetchCep = async (cepValue) => {
        ...
        setLoadingCep(true);
        try {
            // VOLTANDO PARA A FORMA NÃO SEGURA (CHAMADA EXTERNA DIRETA)
            // Aviso de Arquiteto: Isso remove a proteção de Cache e Rate Limiting do seu backend.
            const response = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
            const result = await response.json();

            // A estrutura de erro do ViaCEP retorna 'result.erro === true'
            if (!result.erro) {
                setData(prevData => ({
                    ...prevData,
            ...
            // Erros de rede ou bloqueio de CORS cairão aqui
            console.error("Erro ao consultar CEP externamente:", error);
